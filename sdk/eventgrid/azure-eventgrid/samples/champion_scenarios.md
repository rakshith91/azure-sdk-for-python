...
client.send_events([event0, event1])
```
...
deserialized_dict_events = client.deserialize_eventgrid_events(eg_storage_dict)

# to return raw json, we use the data param
for event in deserialized_dict_events:
	print(event.data)
	print(type(event.data)) # this is a dictionary

	# use system_event_data to get a strongly typed model
	print(event.system_event_data)
	print(type(event.system_event_data)) # this is a strongly typed model or None
```
...
client = EventGridDeserializer()
deserialized_dict_events = client.deserialize_cloud_events(cloud_custom_dict)
...
for event in deserialized_dict_events:
	...
	# use system_event_data to get a strongly typed model
	print(event.system_event_data)
	print(type(event.system_event_data)) # this is None because type is not recognized system event.
```